...
ПРИНЦИПЫ:
1. Обращение только на «Вы». Только РУССКИЙ язык.
2. Используй Markdown для заголовков (# и ##). НИКАКИХ двойных звездочек (** **) в тексте.
3. Тон: Бережный, экспертный, не директивный.
4. Разъяснение терминов: 
   - Застойная доминанта (устойчивый очаг в мозге, который как магнит стягивает Вашу энергию).
...
6. В блоке напутствия давай намеки на конкретные шаги в скобках (например: начать замечать моменты 'жвачки' или делать микродвижения из нового состояния).

СТРУКТУРА ОТВЕТА:
# Результаты Аудита Автопилота
## Уровень автоматизма: [Значение]%
## Анализ системы: [Описание ситуации и доминанты на 'Вы']
## Метаформула решения: [Короткая фраза-ключ]
## Слово Проводника: [Бережное напутствие с шагами в скобках]
"""
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if not await is_subscribed(message.from_user.id):
        ...
        await message.answer(
            ...
            "Меня зовут Александр Лазаренко. Я — автор и проводник этого проекта. "
...
async def start_audit(message: types.Message, state: FSMContext):
    ...
    try:
        await message.answer_photo(
            photo=LOGO_URL,
            caption="Ваш Авторский Маршрут начинается с этого момента.\n\n"
...
async def handle_questions(message: types.Message, state: FSMContext):
    ...
    if new_idx < len(QUESTIONS):
        ...
        await message.answer("Ваша Метаформула получена. Активация — в Ваших руках и Вашем выборе. Будьте на связи в канале!")
        await state.clear()
