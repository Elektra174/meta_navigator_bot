...
ADMIN_ID = 7830322013  # Ваш ID в Telegram
...
5. Давай намеки на конкретные шаги в скобках (например: начать замечать моменты 'мыслительной жвачки' или делать микродвижения из нового состояния).
...
async def send_report_to_admin(user_info: types.User, answers: list, report: str):
    ...
    try:
        ...
        user_details = (
            ...
            f"• Контакт для связи: [Написать сообщение](tg://user?id={user_info.id})\n\n"
            "📝 *Оригинальные ответы:*\n"
        )
        
        # Добавляем все ответы пользователя
        for answer in answers:
            user_details += f"• {answer}\n"
        
        user_details += f"\n📊 *Сгенерированный отчет AI:*\n\n{report}"
        ...
        print(f"Отчет отправлен администратору от пользователя {user_info.id}")
        return True
    except Exception as e:
        print(f"Ошибка при отправке отчета администратору: {e}")
        return False
        ...
        try:
            await message.answer_photo(
                ...
                caption="Добро пожаловать в «Метаформулу Жизни».\n\n"
                        "Меня зовут Александр Лазаренко.  Я — автор и проводник проекта «Метаформула Жизни». Я помогу Вам увидеть программы Вашего Автопилота и проложить маршрут к себе настоящему.\n\n"
...
async def handle_questions(message: types.Message, state: FSMContext):
    ...
    if new_idx < len(QUESTIONS):
        ...
        # Сохраняем данные пользователя перед очисткой состояния
