...
# Настройка сигналов для корректной работы на Linux (Render)
if sys.platform != 'win32':
    signal.signal(signal.SIGALRM, signal.SIG_IGN)
    signal.signal(signal.SIGCHLD, signal.SIG_IGN)

# Загрузка ключа Firebase из переменных окружения
...
# Проверка доступности Cerebras AI
try:
...
# =================================================================================================
# 2. ФУНКЦИИ ОБЛАЧНОГО ХРАНЕНИЯ (FIRESTORE)
# =================================================================================================
...
QUESTIONS = [
    ...
    "📍 **Точка 7: Реинтеграция.**\nКакое качество в поведении других людей тебя раздражает сильнее всего? Если представить, что за этим стоит какая-то скрытая сила — что это за сила и как бы ты мог использовать её себе на пользу?",
...
<head>
    ...
    <style>
        ...
        .btn {{ background: linear-gradient(135deg, #b4932c 0%, #D4AF37 100%); color: black; font-weight: 800; padding: 16px 40px; border-radius: 8px; text-transform: uppercase; display: inline-block; text-decoration: none; }}
...
<body class="p-6 md:p-12 max-w-5xl mx-auto">
    <header class="text-center mb-16 border-b border-gray-900 pb-10">
        ...
        <p class="text-xl text-gray-500 mt-4 tracking-widest font-mono">КАРТА ДЕШИФРОВКИ: {user_name}</p>
    </header>
...
async def process_answers(message: types.Message, state: FSMContext):
    ...
    if step + 1 < len(QUESTIONS):
        ...
        await message.answer("🎯 Аудит завершен. Твой персональный веб-отчет готов:", reply_markup=kb.as_markup())
...
# =================================================================================================
# 7. ВЕБ-СЕРВЕР (ГЕНЕРАЦИЯ СТРАНИЦ)
# =================================================================================================

async def handle_home(request):
    return web.Response(text="Identity Lab System v6.4 ONLINE", content_type='text/plain')

async def handle_report(request):
    """Выдача персональной веб-страницы отчета"""
    try:
        ...
        return web.Response(text="Отчет не найден. Пожалуйста, пройдите аудит в боте.", status=404)
    except Exception as e:
