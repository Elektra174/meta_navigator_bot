...
async def check_btn(callback: types.CallbackQuery, state: FSMContext):
    if await is_subscribed(callback.from_user.id):
        await callback.message.answer("Доступ открыт. Начинаем аудит твоего автопилота.")
...
async def handle_questions(message: types.Message, state: FSMContext):
    ...
    if new_idx < len(QUESTIONS):
        ...
        await message.answer("Твой Авторский Маршрут начинается здесь. Будь на связи в канале проекта!")
        await state.clear()
